...
pub fn check_crate(cm: &CodeMap, span_handler: &SpanHandler, krate: &ast::Crate,
                   plugin_attributes: &[(String, AttributeType)], unstable: UnstableFeatures) -> Features
{
    ...
    if !allow_features {
        for attr in &krate.attrs {
            if attr.check_name("feature") {
                ...
                span_handler.span_err(attr.span, &format!("#[feature] may not be used on the {} release channel", release_channel));
            }
        }
    }
}
