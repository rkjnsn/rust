...
fn spawn_raw_newsched(mut opts: TaskOpts, f: ~fn()) {
    ...
    let mut task = unsafe {
        if opts.sched.mode != SingleThreaded {
            ...
            let join_task = do Local::borrow::<Task, ~Task>() |running_task| {
                let thread_port_cell = Cell::new(thread_port_cell.take());
                ~do running_task.new_child(&mut (*new_sched).stack_pool) {
