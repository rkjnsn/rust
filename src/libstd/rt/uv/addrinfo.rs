...
type GetAddrInfoCallback = ~fn(GetAddrInfoRequest, &AddrInfo, Option<UvError>);

pub struct GetAddrInfoRequest(*uvll::uv_getaddrinfo_t);
pub struct AddrInfo(*uvll::addrinfo);
...
impl GetAddrInfoRequest {
    ...
    pub fn getaddrinfo(&mut self, loop_: &Loop, node: Option<&str>,
                       service: Option<&str>, hints: Option<AddrInfo>,
...
mod test {
    ...
    fn getaddrinfo_test() {
        ...
        do req.getaddrinfo(&loop_, Some("localhost"), None, None) |_, _, _| {
            error!("doin a callback, y'all");
        }
        loop_.run();
        loop_.close();
        req.delete();
