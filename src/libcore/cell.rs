...
//! shared `&` references, whereas most Rust types can only be mutated
//! through `&mut` references. We say that `Cell` and `RefCell`
//! provide *interior mutability*, in contrast with typical Rust types
//! that exhibit *inherited mutability*.
//!
//! Cell types come in two flavors: `Cell` and `RefCell`, with
//! different runtime semantics. `Cell` provides `get` and
//! `set` methods that atomically change the interior value with a
//! single method call. `Cell` though is only compatible with types
//! that implement `Copy` and are therefore guaranteed not to have
//! unexpected side effects. For other types, one must use the
//! `RefCell` type, acquiring a write lock before mutating.
//!
...
//!
//! The more common inherited mutability is one of the key components
//! that enables Rust to reason strongly about pointer
//! aliasing. Because of that, interior mutability is something of a
//! last resort. Since cell types enable mutation where it would
//! otherwise be disallowed though, there are occassions when interior
//! mutability might be appropriate, or even *must* be used.
//!
...
//!
// FIXME: This example could be better
//! ```
...
//! ```
//!
//! ## When mutation is an implementation detail of `clone`
//!
...
//! ```
//!
//! # Detail about differences between Cell, RefCell, and dynamic borrowing
//! TODO: Explain difference between Cell and RefCell
//!
//! # Downsides to interior mutability
//!
